...
export type ParishSettings = {
  ...
  memo_thermal_font_scale?:number; memo_show_notes?:number;
...
export async function saveSettings(v: ParishSettings) {
  ...
  await (await db()).execute("UPDATE parish_settings SET memo_thermal_font_scale=$1,memo_show_notes=$2 WHERE id=1",[v.memo_thermal_font_scale??115,v.memo_show_notes??0]);
